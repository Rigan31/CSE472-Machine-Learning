...
class FullyConnectedLayer:
    def __init__(self, output_size, learning_rate=0.0001):
...
class ConvoModel:
    ...
    def __init__(self, learning_rate=0.0001, batch_size=32):
        ...
        self.fullyConnectedLayer1 = FullyConnectedLayer(output_size=10, learning_rate=0.0001)
...
def load_images(data_folder):
    ...
    for file_name in tqdm(os.listdir(data_folder)):
        ...
        image = cv2.imread(os.path.join(data_folder, file_name), 0)
...
def preprocess_images(images, size):
    ...
    for images in tqdm(images):
        image = cv2.resize(images, size, interpolation=cv2.INTER_CUBIC)
...
for epoch in range(epochs):
    ...
    print("true data:", validation_y)
    print("validation data:", validation_pred)
