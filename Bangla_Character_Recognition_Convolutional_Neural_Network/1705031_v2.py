...
class ConvolutionLayer:
    ...
    def forward(self, u):
        ...
        if self.weights is None:
            ...
            self.weights = np.random.randn(self.num_filters, self.filter_shape, self.filter_shape,
                                           num_channels) * math.sqrt(
                2 / (self.filter_shape * self.filter_shape * num_channels))
    ...
    def backward(self, del_v):
        ...
        for l in range(self.num_filters):
            for i in range(self.filter_shape):
                for j in range(self.filter_shape):
                    del_w[l, i, j, :] = np.mean(np.sum(self.input_pad[:, i: i + input_dim_pad, j: j + input_dim_pad, :] * np.reshape(del_v_sparse[:, :, :, l], del_v_sparse.shape[: 3] + (1,)), axis=(1, 2)), axis=0)

        del_u = np.zeros((num_samples, output_dim, output_dim, num_channels))
        del_v_sparse_pad = np.pad(del_v_sparse, ((0,), (self.filter_shape - 1 - self.padding,), (self.filter_shape - 1 - self.padding,), (0,)), mode='constant')

        for k in range(num_samples):
            for l in range(num_channels):
                for i in range(output_dim):
                    for j in range(output_dim):
                        del_u[k, i, j, l] = np.sum(del_v_sparse_pad[k, i: i + self.filter_shape, j: j + self.filter_shape, :] * weights_prime[l])
        ...
        print("del_w convo layer: ", del_w[0, 0, 0, 0])
        print("del_b convo layer: ", del_b[0])
            ...
            for h in range(output_height):
                for w in range(output_width):
                    for c in range(input_channel):
                        output[i, h, w, c] = np.max(input[i, h * self.stride: h * self.stride + self.filter_shape, w * self.stride: w * self.stride + self.filter_shape, c])
                        self.save_output[i, h, w, c] = np.argmax(input[i, h * self.stride: h * self.stride + self.filter_shape, w * self.stride: w * self.stride + self.filter_shape, c])
...
class ConvoModel:
    ...
    def __init__(self, learning_rate=0.001):
        ...
        self.convolutionLayer = ConvolutionLayer(self.input_channels, self.output_channels, self.filter_dim, self.stride, self.padding)
        self.reluLayer = ReLUActivation()
        self.maxPoolingLayer = MaxPoolingLayer(filter_shape=2, stride=2)
        self.flattenLayer = FlattenLayer()
        self.fullyConnectedLayer = FullyConnectedLayer(output_size=10, learning_rate=0.001)
    ...
    def forward(self, x):
        ...
        x = self.fullyConnectedLayer.forward(x)
    ...
    def backward(self, delta):
        ...
        delta = self.fullyConnectedLayer.backward(delta)
...
def train_test_data():
    ...
    train_x, test_x = preprocess_images_list[0:train_size], preprocess_images_list[train_size:len(preprocess_images_list)]
...
epochs = 10
lr = 0.0001
model = ConvoModel(learning_rate=lr)
